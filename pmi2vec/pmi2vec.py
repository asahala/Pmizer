            ...
            for word in self.word_freqs.keys():
                ...
                if vecs:
                    self.word_vectors[word.replace('+', '')] = (self.word_freqs[word],
                                                                list(np.mean(vecs, axis=0)))
        else:
            for morph in self.morph_freqs.keys():
                index = self.index_table.get(morph, None)
                if index is not None:
                    self.word_vectors[morph] = (self.morph_freqs[morph], list(matrix[index]))
...
def demo():
    ...
    a.score_bigrams(txt, measure=PPMI, windowsize=5, symmetry=True, minfreq=2)
...
def query():
    ...
    for emotion in ['râmu_1','adāru_2','galātu_1',
                    'palāhu_1','parādu_1','šahātu_1',
                    'agāgu_1','ezēzu_1','kamālu_1',
                    'labābu_1','raʾābu_1','šabāsu_1',
                    'šamāru_1','zenû_1']:
        print('\n')
        print(emotion)
        a.get_best(emotion, minfreq=5, amount=10)
